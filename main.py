...
#     This endpoint checks if a profile already exists for the user.
...
def create_company_profile(
    ...
    try:
        ...
        # 🔥 MySQL UPSERT (fix duplicate error)
        stmt = insert(models.CompanyProfile).values(
            id=str(uuid4()),
            user_id=data.user_id,
            data=profile_data
        )
